    ...
    if not session.get('user'):
        if request.method=='POST':
            ...
            if bdata[0]==1:
                ...
                if password==bpassword[0].decode('utf-8'): #'111'=='111'
                    print(session)
                    session['user']=uemail
                    print(session)
    ...
    if session.get('user'):
        if request.method=='POST':
            ...
            if uid:
                ...
                return 'somethig went wrong to fetch uid'
...
def viewallnotes():
    if session.get('user'):
        try:
            ...
            notesdata=cursor.fetchall() #[( 1 , 'python', '2024-12-14 16:51:57'),(3,'python1 ', '2024-12-14 16:52:41'),( 5,'mysql','2024-12-14 16:56:33')]
            cursor.close()
        except Exception as e:
            print(e)
            flash('No data found ')
...
def readnotes(nid):
    if session.get('user'):
        try:
            ...
            notesdata=cursor.fetchone()  #(1,'python','interpreted language and is high level','2024-12-14 16:51:57',3)
            cursor.close()
        except Exception as e:
            print(e)
